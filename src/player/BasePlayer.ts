...
export abstract class BasePlayer extends TypedEmitter<PlayerEvents> {
    ...
    private static getVideoSettingFromStorage(
      ...
      const {
        crop, bitrate, iFrameInterval, sendFrameMeta, lockedVideoOrientation,
      } = parsed;
    ...
    private drawStats(): void {
      ...
      if (this.perSecondQualityStats && this.momentumQualityStats) {
        const {
          decodedFrames, droppedFrames, inputBytes, inputFrames,
        } = this.momentumQualityStats;
        const {
          avgDecoded, avgDropped, avgSize, avgInput,
        } = this.perSecondQualityStats;
