...
export default class FilePushHandler implements DragEventListener {
    ...
    private logError(pushId: number, fileName: string, code: number): void {
      ...
      this.sendUpdate({
        pushId, fileName, logString: `error: "${msg}"`, error: true,
      });
    }
    ...
    private async pushFile(file: File): Promise<void> {
      ...
      this.sendUpdate({
        pushId: id, fileName, logString: 'begin...', error: false,
      });
      ...
      const startParams = {
        id: pushId, fileName, fileSize, state: FilePushState.START,
      };
      ...
      const processData = async ({ done, value }: { done: boolean; value?: any }): Promise<void> => {
        if (done) {
          ...
          if (finishResponseCode !== 0) {
            ...
            this.sendUpdate({
              pushId, fileName, logString: 'success!', error: false,
            });
          }
        ...
        this.sendUpdate({
          pushId, fileName, logString: `${percent.toFixed(2)}%`, error: false,
        });
